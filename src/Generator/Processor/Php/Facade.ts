...
export default class Facade extends Default {
    ...
    generatePhpClass = (resourceConfig: Resource): string => {
        ...
        const className = `${targetClass} extends AbstractFacade implements ${targetClass}Interface`;

        let classCode = `<?php\n\n`;

        classCode += `namespace ${namespace};\n\n`;

        classCode += `use Spryker\\Zed\\Kernel\\Business\\AbstractFacade;\n`;
