...
export default class BusinessFactory extends Default {
    ...
    generatePhpClass = (resourceConfig: Resource): string => {
        ...
        const className = `${targetClass} extends AbstractBusinessFactory`;

        let classCode = `<?php\n\n`;

        classCode += `namespace ${namespace};\n\n`;

        classCode += `use Spryker\\Zed\\Kernel\\Business\\AbstractBusinessFactory;\n`;
